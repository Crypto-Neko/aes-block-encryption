...
class AES:
    ...
    # Get the inverse s_box as defined for AES
    def get_s_box_inv(self):
        s_box_inv = [
    ...
    def sub_bytes(self, state):
        ...
        sub_function = lambda x : self.s_box[x // 16, x % 16]
        vector_function = np.vectorize(sub_function)
    ...
    def shift_rows(self, state):
        # Shift ("roll") the rows in each state using NumPy
        state = deepcopy(state)
        state[1] = np.roll(state[1], -1)
        state[2] = np.roll(state[2], -2)
        state[3] = np.roll(state[3], -3)

        # Return the result after shifting the rows
        return state

    # Invert row shifting
    def inv_shift_rows(self, state):
        # Shift ("roll") the rows back to reverse the row shifting
        state = deepcopy(state)
        state[1] = np.roll(state[1], 1)
        state[2] = np.roll(state[2], 2)
        state[3] = np.roll(state[3], 3)

        # Return the state after reversing the row shifting
        return state
